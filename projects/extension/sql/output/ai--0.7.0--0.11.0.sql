...
begin
    ...
    values (_migration_name, _migration_body, $version$0.11.0$version$);
end;
...
as $python$
    if "ai.version" not in GD:
        ...
        python_lib_dir = Path(python_lib_dir).joinpath("0.11.0")
        import site
        site.addsitedir(str(python_lib_dir))
        from ai import __version__ as ai_version
        assert("0.11.0" == ai_version)
        GD["ai.version"] = "0.11.0"
    else:
        if GD["ai.version"] != "0.11.0":
