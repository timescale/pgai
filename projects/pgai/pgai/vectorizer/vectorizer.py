...
class Worker:
            ...
            try:
                ...
                while True:
                    ...
                    if self.vectorizer.config.embedding.is_api_async():
                        ...
                        # TODO: this should only exit if there are not more
                        # items in the queue. This logic shouldn't be here
                        # probably is going to change with s3 branch
                        return res
    ...
    async def check_and_store_async_batches(self, conn: AsyncConnection):
        ...
        async with conn.cursor() as cursor:
            ...
            for batch_row in await cursor.fetchall():
                ...
                batch = self.vectorizer.config.embedding.fetch_async_embedding_status(
                    batch
                )

                with conn.transaction():
                    await conn.execute(
                        self.queries.update_batch_embedding_query,
                        (
                            batch.status,
                ...
                if batch.status == "completed":
                    with conn.transaction():
                        await self.vectorizer.config.embedding.write_embeddings_from_batch(
                            conn, batch
                        )
                ...
                if batch.status == "processed":
                    with conn.transaction():
                        await self.vectorizer.config.embedding.finalize_async_embedding(
                            batch
                        )
                        await cursor.execute(
    ...
    async def create_async_batches(self, conn: AsyncConnection) -> int:
        ...
        try:
            ...
            await conn.execute(
                self.queries.insert_batch_embedding_query,
                (
            ...
            for doc in documents:
                await conn.execute(
                    ...
                    (doc["unique_full_chunk_id"], created_batch.id, doc["chunk"]),
                )
    ...
    async def _generate_embedding_batch(
        ...
        try:
            batch = await self.vectorizer.config.embedding.create_and_submit_embedding_batch(
                documents
            )
