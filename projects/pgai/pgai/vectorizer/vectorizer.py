...
class VectorizerQueryBuilder:
    ...
    def fetch_work_query_with_split_transaction(self) -> sql.Composed:
        ...
        The query will take an advisory lock on all rows returned by the query.
        The advisory lock must be manually unlocked when the rows have been
        processed.
...
class Executor:
    ...
    async def run(self) -> int:
        ...
        # Ensure that we do not hold advisory locks for longer than intended:
        # Explicitly configure keepalive parameters so that the server closes
        # connections if our client becomes unresponsive.
        keepalive_params: dict[str, Any] = {
            "keepalives": 1,
            "keepalives_idle": 50,
