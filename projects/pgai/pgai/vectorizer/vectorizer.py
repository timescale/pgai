...
from typing import Any, Tuple, TypeAlias, TypeVar, cast
...
class VectorizerQueryBuilder:
    ...
    def pk_column_definition_list_sql(self) -> sql.Composed:
        ...
        return sql.SQL(", ").join(
            [
                sql.SQL(" ").join([sql.Identifier(a.attname), sql.SQL(cast(LiteralString, a.typname))])
                for a in sorted(self.vectorizer.source_pk, key=lambda pk: pk.attnum)
            ]
        )
    ...
    def fetch_work_query_with_split_transaction(self) -> sql.Composed:
        ...
        return sql.SQL("""
                WITH locked_queue_rows AS materialized (
                    SELECT ctid, _lock_id, {pk_fields}, attempts, retry_after
                    FROM ai._get_next_queue_batch('{queue_table}', %s) as (ctid tid, _lock_id int, {pk_columns}, queued_at timestamptz, attempts int, retry_after timestamptz)
                ),
    ...
    def delete_processed_queue_rows_query(self, items: Sequence[SourceRow]) -> Tuple[sql.Composed, list[Any]]:
        """
        ...
        query = sql.SQL(
            ...
            queue_table=sql.Identifier(self.vectorizer.queue_schema, self.vectorizer.queue_table),
            pk_fields=self.pk_fields_sql,
    ...
    def update_retry_after_and_move_permanent_failures_to_dlq_query(self, max_attempts: int, items: Sequence[Tuple[SourceRow, ProcessingError]]) -> Tuple[sql.Composed, list[Any]]:
        """
        ...
        query = sql.SQL("""
            ...
            """).format(
            error_values=self._placeholders_tuples(len(self.vectorizer.source_pk) + 1, len(items)),
            pk_fields=self.pk_fields_sql,
            queue_table=sql.Identifier(
                self.vectorizer.queue_schema, self.vectorizer.queue_table
            ),
...
class Executor:
    ...
    async def _do_batch(self, conn: AsyncConnection) -> int:
        ...
        await logger.adebug(f"Fetched {len(items)} rows from queue", batch_size=self._batch_size)
    ...
    async def _embed_and_write(self, conn: AsyncConnection, items: list[SourceRow]):
        ...
        async for records, errors in self._generate_embeddings(items):
            ...
            count += len(records)
        if self.features.split_processing:
            await self._delete_processed_queue_rows(conn, [item for item in items if item not in failed_items])
            await conn.commit()
        return count
    ...
    async def _delete_embeddings(self, conn: AsyncConnection, items: list[SourceRow]):
        ...
        if self.vectorizer.config.destination.implementation == "column":
            ...
            async with conn.cursor() as cursor, timed_debug_log(f"Removed embeddings for {len(items)} source rows"):
                await cursor.execute(
    ...
    @tracer.wrap()
    async def _delete_processed_queue_rows(self, conn: AsyncConnection, items: list[SourceRow]):
        """
        ...
        if items:
            ...
            async with conn.cursor() as cursor, timed_debug_log(f"Removed {len(items)} processed queue rows"):
                await cursor.execute(
                    query,
                    params
                )
    ...
    async def _update_source_table(
        ...
        async with conn.cursor() as cursor, timed_debug_log(f"UPDATED {len(records)} embeddings"):
            for record in records:
    ...
    async def _generate_embeddings(
        ...
        try:
            ...
            async for embeddings in self.vectorizer.config.embedding.embed(documents):
                ...
                duration = now - start
                start = now
                await logger.adebug(f"obtained {len(records)} embeddings in {duration:.2}s")
                yield records, []
        except Exception as e:
    ...
    async def _handle_error(self, conn: AsyncConnection, items: Sequence[SourceRow], error: ProcessingError):
        await self._set_retry_after_and_move_to_dlq(conn, [(item, error) for item in items])
        await self._insert_vectorizer_error(
            conn, self._format_error(error)
        )
    ...
    async def _set_retry_after_and_move_to_dlq(self, conn: AsyncConnection, errors: Sequence[tuple[SourceRow, ProcessingError]]):
        query, params = self.queries.update_retry_after_and_move_permanent_failures_to_dlq_query(6, errors)

        async with conn.cursor() as cursor, timed_debug_log(f"set retry_after and moved to dlq on {len(errors)} items"):
            await cursor.execute(
                query,
                params
            )
