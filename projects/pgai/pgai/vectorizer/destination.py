...
class DestinationProtocol(Protocol):
    ...
    def get_copy_embeddings_query(
        self, target_table_ident: sql.Identifier, pk_fields_sql: sql.Composed
    ) -> sql.Composed:
...
class DefaultDestination(BaseModel):
    ...
    def get_copy_embeddings_query(
        ...
        """Returns a SQL query for COPY operation to insert embeddings into
        the target table"""
        return sql.SQL(
...
class SourceDestination(BaseModel):
    ...
    def get_update_embedding_query(
        ...
        """Returns a SQL query to update the embedding column
