--------------------------------------------------------------------------------
-- ai 0.10.1 (x-release-please-version)
...
-------------------------------------------------------------------------------
-- 030-semantic-catalog.sql
do $outer_migration_block$ /*030-semantic-catalog.sql*/
declare
    _sql text;
    _migration record;
    _migration_name text = $migration_name$030-semantic-catalog.sql$migration_name$;
    _migration_body text =
$migration_body$

create table ai.semantic_catalog
( id int4 not null primary key generated by default as identity 
, catalog_name text not null unique check (catalog_name ~ '^[a-z][a-z_0-9]*$')
);
