...
class ChunkingConfig:
    ...
    def to_sql_argument(self) -> str:
        ...
        return f", chunking => ai.chunking_recursive_character_text_splitter({', '.join(params)})"
...
class DiskANNIndexingConfig:
    ...
    def to_sql_argument(self) -> str:
        ...
        if self.create_when_queue_empty is not None:
            params.append(f"create_when_queue_empty=>{str(self.create_when_queue_empty).lower()}")
...
class CreateVectorizerParams:
    ...
    def to_sql(self) -> str:
        parts = [
            ...
            f"'{self.source_table}'::regclass"
        ]

        # Handle all config objects that implement to_sql_argument
        for field in fields(self):
            value = getattr(self, field.name)
            if isinstance(value, SQLArgumentProvider) and value is not None:
                parts.append(value.to_sql_argument())

        # Handle string parameters
        string_fields = ['destination', 'target_schema', 'target_table',
                         'view_schema', 'view_name', 'queue_schema', 'queue_table']
        ...
        if self.formatting_template is not None:
            parts.append(f", formatting => ai.formatting_python_template('{self.formatting_template}')")
    ...
    def to_python(self, autogen_context: AutogenContext) -> str:
        ...
        for field in fields(self):
            value = getattr(self, field.name)
            if isinstance(value, PythonArgProvider) and value is not None:
