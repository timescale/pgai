...
def test_named_vectorizer_timescaledb():
    ...
    with psycopg.connect(
        ...
        with con.cursor() as cur:
            ...
            with pytest.raises(psycopg.errors.InternalError_) as exc_info:
                cur.execute("select ai.execute_vectorizer('non_existent_vectorizer')")
            assert "Vectorizer with name 'non_existent_vectorizer' not found" in str(
                exc_info.value
            )

            cur.execute("alter table ai.vectorizer drop column name cascade;")
            with pytest.raises(psycopg.errors.InternalError_) as exc_info:
                cur.execute("select ai.execute_vectorizer('blog_vectorizer')")
            assert (
