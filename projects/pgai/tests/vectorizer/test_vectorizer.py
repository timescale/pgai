...
def test_vectorizer_internal():
    ...
    with (
        ...
        cur.execute(
            SQL("""
                select count(*) = count(*) filter (where chunk = format('%s: %s', id, note)) 
                from {view_schema}.{view_name}
                """).format(
                view_schema=Identifier(vectorizer_expected.view_schema),
                view_name=Identifier(vectorizer_expected.view_name),
            )
        )
        actual = cur.fetchone()[0]  # type: ignore
