
set local search_path = pg_catalog, pg_temp;

/*
if this is the first installation of the extension, create the schema and
migration table.
if the extension has already been installed (i.e. we are upgrading), make sure
that the user doing the upgrade is the same user that owns the schema and
migration table. abort the upgrade if different.
*/
do $bootstrap_extension$
declare
    _current_user_id oid = null;
    _schema_owner_id oid = null;
    _migration_table_owner_id oid = null;
begin
    select pg_catalog.to_regrole('@extowner@')::oid
    into strict _current_user_id;

    select pg_namespace.nspowner into _schema_owner_id
    from pg_catalog.pg_namespace
    where pg_namespace.nspname operator(pg_catalog.=) '@extschema@';

    if _schema_owner_id is null then
        create schema @extschema@;
    elseif _schema_owner_id is distinct from _current_user_id then
        raise exception 'only the owner of the @extschema@ schema may upgrade the extension';
        return;
    end if;

    select k.relowner into _migration_table_owner_id
    from pg_catalog.pg_class k
    inner join pg_catalog.pg_namespace n on (k.relnamespace = n.oid)
    where k.relname operator(pg_catalog.=) 'migration'
    and n.nspname operator(pg_catalog.=) '@extschema@';

    if _migration_table_owner_id is not null
    and _migration_table_owner_id is distinct from _current_user_id then
        raise exception 'only the owner of the @extschema@.migration table can upgrade this extension';
        return;
    end if;

    if _migration_table_owner_id is null then
        create table @extschema@.migration
        ( "name" text not null primary key
        , applied_at_version text not null
        , applied_at timestamptz not null default pg_catalog.clock_timestamp()
        , body text not null
        );
        perform pg_catalog.pg_extension_config_dump('@extschema@.migration'::pg_catalog.regclass, '');
    end if;
end;
$bootstrap_extension$;

