...
Dataset Setup:
1. Use paul_graham_essays.csv from HuggingFace
2. Import to PostgreSQL: `docker cp "/path/to/paul_graham_essays.csv" pgai-db-1:/tmp/`
3. Create vectorizers for each model
4. Verify vectorization status and dimensions
...
class Config:
    ...
    EMBEDDING_TABLES = [
        'essays_nomic_embeddings',
        'essays_openai_small_embeddings',
        'essays_bge_large_embeddings',
        'essays_openai_large_embeddings'
    ]
...
class DatabaseConnection:
    def __init__(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute("SET ai.openai_api_key = %s", (self.api_key,))
    ...
    def vector_search(self, table: str, question: str, k: int) -> List[Tuple[int, int]]:
        ...
        # This shouldn't get triggered since I fixed the bug, but it's a good fallback
        ...
        with self.conn.cursor() as cur:
            if 'nomic' in table:
                cur.execute(f"""
                    SELECT id, chunk_seq 
                    FROM {table} 
                    ORDER BY embedding <=> ai.ollama_embed('nomic-embed-text', %s, host => 'http://ollama:11434')
                    LIMIT %s
                """, (question, k))
            elif 'openai_small' in table:
                cur.execute(f"""
                    SELECT id, chunk_seq 
                    FROM {table} 
                    ...
                    LIMIT %s
                """, (question, k))
            elif 'openai_large' in table:
                cur.execute(f"""
                    SELECT id, chunk_seq 
                    FROM {table} 
                    ORDER BY embedding <=> ai.openai_embed('text-embedding-3-large', %s, dimensions => 1536)
                    LIMIT %s
                """, (question, k))
            elif 'bge' in table:
                cur.execute(f"""
                    SELECT id, chunk_seq 
                    FROM {table} 
                    ORDER BY embedding <=> ai.ollama_embed('bge-large', %s, host => 'http://ollama:11434')
                    LIMIT %s
                """, (question, k))
